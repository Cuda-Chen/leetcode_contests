// https://leetcode.com/problems/escape-the-spreading-fire/discuss/1994556/C%2B%2B-or-BFS-%2B-Binary-Search
// BFS + binary search
...
private:
    ...
    bool possible(int r, int c, int t, vector<vector<int>> &visited, vector<vector<int>> &fire) {
        ...
        for(int i = 0; i < 4; i++) {
            int nr = r + dir[i], nc = c + dir[i + 1];
            if(nr >= 0 && nr < fire.size() && nc >= 0 && nc < fire[0].size() &&
               visited[nr][nc] > t + 1 &&
               fire[nr][nc] >= t + 1 &&
               possible(nr, nc, t + 1, visited, fire))
                return true;
        }
        return false;
    }
