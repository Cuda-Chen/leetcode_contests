...
public:
    long long minimalKSum(vector<int>& A, int k) {
        ...
        for(int i = 0; i < N && k; i++) {
            ...
            k -= cnt; // use the `cnt` missing numbers
            ans += (long)(prev + 1 + prev + cnt) * cnt / 2; // sum of these `cnt` missing numbers `[prev + 1, prev + cnt]`.
        }
        if(k > 0) ans += ((long)A.back() + 1 + A.back() + k) * k / 2;// If there are still missing numbers, add the sum of numbers `[A.back() + 1, A.back() + k]` to anwser
        return ans;
    }
};
